...
class DataManager:
    ...
    def _upsert_persisted_rating(self, user_id: int, movie_id: int, rating: float) -> float:
        """Guarda o actualiza el rating en train_ratings.json y devuelve la media del usuario."""
        ratings_path = self.data_path / "train_ratings.json"
        ratings_data = []
        ...
        for entry in ratings_data:
            if (
                int(entry.get("userId", -1)) == int(user_id)
                and int(entry.get("movieId", -1)) == int(movie_id)
            ):
        ...
        with ratings_path.open("w", encoding="utf-8") as file:
            json.dump(ratings_data, file, ensure_ascii=False, indent=2)

        user_ratings = [
            float(entry["rating"])
            for entry in ratings_data
            if int(entry.get("userId", -1)) == int(user_id)
        ]
        if not user_ratings:
            return float(rating)
        return float(sum(user_ratings) / len(user_ratings))

    def _update_registry_genre_vector(
        self,
        user_data: dict,
        movie_id: int,
        rating: float,
    ) -> dict:
        """Actualiza el genre_vector del usuario con los generos de la pelicula puntuada."""
        preferences = user_data.get("preferences", {}) if isinstance(user_data, dict) else {}
        favorite_genres = preferences.get("favorite_genres", [])
        existing_vector = preferences.get("genre_vector", {})

        genre_vector = {
            str(genre): float(value)
            for genre, value in existing_vector.items()
        ...
        for favorite in favorite_genres:
            ...
            if genre_name:
                genre_vector.setdefault(genre_name, 60.0)

        movie_data = self.movies.get(str(movie_id), {})
        genre_ids = movie_data.get("generos", []) if isinstance(movie_data, dict) else []
        genre_names = self.get_genre_names(genre_ids) if isinstance(genre_ids, list) else []
        delta = (float(rating) - 2.5) * 20.0

        for genre_name in genre_names:
            current = float(genre_vector.get(genre_name, 0.0))
            updated = max(0.0, min(100.0, current + delta))
            genre_vector[genre_name] = round(updated, 2)
    ...
    def update_user_rating(self, user_id, movie_id, rating):
        ...
        try:
            ...
            self.user_registry_manager.reload_registry()
            user_data = self.user_registry_manager.get_user(user_id)
            if user_data is None:
                st.error(f"❌ Usuario {user_id} no encontrado en el registro")
                return False
            ...
            updated_avg_rating = self._upsert_persisted_rating(user_id, movie_id, rating)
            updated_genre_vector = self._update_registry_genre_vector(
                user_data=user_data,
                movie_id=movie_id,
                rating=rating,
            )
            ...
            self.user_registry_manager.update_user_activity(
                ...
                new_avg_rating=updated_avg_rating,
                watched_movies=watched_movies,
                genre_vector=updated_genre_vector,
                clear_neighbors=True,
            )
            if COLLABORATIVE_MATRIX_PATH.exists():
                COLLABORATIVE_MATRIX_PATH.unlink()
            return True
        except Exception as e:
            error_msg = str(e)
